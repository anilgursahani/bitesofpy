...
def convert_eu_to_us_date(date):
    """Receives a date string in European format of dd/mm/yyyy, e.g. 11/03/2002
       ...
       Take the european date and convert it to a date time object using strptime
       Then using strftime convert it to American date mm/dd/yyyy
    """
    
    theDate = datetime.strptime(date, "%d/%m/%Y")
    usDate = theDate.strftime("%m/%d/%Y")
    return usDate
